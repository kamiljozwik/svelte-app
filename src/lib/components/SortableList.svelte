<script lang="ts">
	...
	// custom event dispatcher. Parent component can listen to this event
	...
	const reorder = ({ from, to }: any) => {
		...
		// parent component can listen to this event and access the data
...
{#if list?.length}
	<ul class="list-none p-0 flex flex-col items-center">
		<!-- (item.id) to coś jak "key" w React -->
		{#each list as item, index (item.id)}
			...
			>
				<!-- slot props. It allows consumer of the component to access these data -->
				<slot {item} {index} />
			</li>
		{/each}
	</ul>
